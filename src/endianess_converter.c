...
uint64_t
byte_arr_to_u64(uint8_t* bytes, size_t size, int endianess)
{
	return endianess == LSB ? lsb_byte_arr_to_u64(bytes, size) : msb_byte_arr_to_u64(bytes, size);
}
...
void
u64_to_byte_arr(uint64_t num, uint8_t result_bytes[8], int endianess)
{
	endianess == LSB ? u64_to_byte_arr_lsb(num, result_bytes) : u64_to_byte_arr_msb(num, result_bytes);
}
